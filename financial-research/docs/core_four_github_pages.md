...
4. 手动运行一次 `Core stock daily report` workflow，让它创建并更新 `gh-pages` 分支。
5. 在 `Settings -> Pages` 中选择 `Build and deployment -> Source -> Deploy from a branch`，分支选 `gh-pages`，目录选 `/ (root)`。
6. 确认 Pages URL 可访问。
